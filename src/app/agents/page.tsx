...
import { Bot, FileText, DollarSign, UserPlus, Search, Filter, Calendar } from "lucide-react";
...
export default function AgentsPage() {
    ...
    // Group activities by Invoice
    const groupedActivities = filteredActivities.reduce((acc, activity) => {
        const key = activity.relatedInvoice && activity.relatedInvoice !== "N/A"
    ...
    const sortedGroups = Object.entries(groupedActivities).sort(([, a], [, b]) => {
        const latestA = Math.max(...a.map(x => new Date(x.timestamp).getTime()));
        const latestB = Math.max(...b.map(x => new Date(x.timestamp).getTime()));
        ...
        <div className="flex flex-col h-full">
            ...
            <div className="border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
                <div className="px-6 py-2 flex flex-col md:flex-row md:items-center gap-4">
                    ...
                    <div className="flex-1 md:ml-auto max-w-sm relative">
                        ...
                        <Input
                            placeholder="Search activities or invoices..."
            ...
            <div className="flex-1 overflow-auto p-6 bg-muted/10">
                <div className="max-w-4xl mx-auto space-y-6">
                    {/* Activity Feed Grouped by Invoice */}
                    {sortedGroups.length === 0 ? (
                        ...
                        sortedGroups.map(([groupTitle, activities]) => (
                            <Card key={groupTitle} className="overflow-hidden border-none shadow-sm bg-background/50 backdrop-blur-sm">
                                <CardHeader className="bg-muted/30 pb-3 py-3 border-b flex flex-row items-center justify-between space-y-0">
